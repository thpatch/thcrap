...
int BP_file_name(x86_reg_t *regs, json_t *bp_info)
{
	...
	file_name = (char**)json_object_get_register(bp_info, regs, "file_name");
	if(!file_name) {
		return 1;
	}
	if(fr.name) {
	...
	fr.local_buffer = stack_gamefile_resolve(fr.name, &fr.local_size);
	...
	if(fr.hooks) {
		...
		fr.local_size += patch_size;
	}
	// Print the type of the file
	if(fr.patch) {
		log_printf("(Patch) ");
	} else if(fr.local_buffer && fr.local_size) {
		log_printf("(External) ");
	} else {
		log_printf("(Archive) ");
	}
...
int BP_file_size(x86_reg_t *regs, json_t *bp_info)
{
	...
	file_size = json_object_get_register(bp_info, regs, "file_size");
	set_patch_size = !(json_is_false(json_object_get(bp_info, "set_patch_size")));
	...
	if(fr.patch) {
		// If we only have a patch and no replacement file, the local size
		// doesn't yet include the actual base file part
		if(!fr.local_buffer) {
			fr.local_size += fr.game_size;
		}
		if(set_patch_size) {
			*file_size = fr.local_size;
		}
	} else if(fr.local_buffer && fr.local_size) {
	...
	if(fr.name) {
		log_printf("(%d bytes)...\n", fr.local_size ? fr.local_size : *file_size);
	}
	return 1;
}
...
int BP_file_size_patch(x86_reg_t *regs, json_t *bp_info)
{
	...
	// ----------
	if(file_size && fr.local_size && fr.game_size != fr.local_size) {
		*file_size = fr.local_size;
	}
	return 1;
}
...
int BP_file_load(x86_reg_t *regs, json_t *bp_info)
{
	// Parameters
	// ----------
	size_t *file_buffer_addr_copy;
	size_t stack_clear_size;
	size_t eip_jump_dist;
	// ----------

	// Registers
	// ---------
	file_buffer_addr_copy = json_object_get_register(bp_info, regs, "file_buffer_addr_copy");
	stack_clear_size = json_object_get_hex(bp_info, "stack_clear_size");
	eip_jump_dist = json_object_get_hex(bp_info, "eip_jump_dist");
	// ----------
	...
	if(!fr.game_buffer || !fr.local_buffer || !fr.local_size) {
		return 1;
	}

	// Let's do it
	memcpy(fr.game_buffer, fr.local_buffer, fr.local_size);

	patchhooks_run(fr.hooks, fr.game_buffer, fr.local_size, fr.game_size, fr.patch, run_cfg);
