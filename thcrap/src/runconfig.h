...
THCRAP_API void runconfig_print();

// Returns the run configuration.
// Use this only for reading a value specific to a plugin.
THCRAP_API const json_t *runconfig_json_get();

// Returns the thcrap directory
THCRAP_API const char *runconfig_thcrap_dir_get();
// Returns the run configuration path
// If there are multiple ones, return only the 1st one
THCRAP_API const char *runconfig_runcfg_fn_get();

// Returns the game id, for example "th06"
THCRAP_API const char *runconfig_game_get();

// Returns the game build, for example "v1.00a"
THCRAP_API const char *runconfig_build_get();

// Returns the command line parameters passed to the exe
THCRAP_API const char *runconfig_cmdline_get();

#ifdef __cplusplus
extern "C++" {
	// Returns the thcrap directory
	THCRAP_API std::string_view runconfig_thcrap_dir_get_view();

	// Returns the game id, for example "th06"
	THCRAP_API std::string_view runconfig_game_get_view();

	// Returns the game build, for example "v1.00a"
	THCRAP_API std::string_view runconfig_build_get_view();
}
#endif
...
// • 3. The plain game ID
// • 4. NULL
THCRAP_API const char *runconfig_title_get();

// Returns the game update url
THCRAP_API const char *runconfig_update_url_get();

// Returns the dat dump path if dat dump is enabled,
// or NULL if it is disabled.
THCRAP_API const char *runconfig_dat_dump_get();

// Returns the patched files dump path if patched files dump is enabled,
// or NULL if it is disabled.
THCRAP_API const char *runconfig_patched_files_dump_get();

// Returns true if the current build in runconfig is a latest build, false otherwise
THCRAP_API bool runconfig_latest_check();

// Returns the "true" latest build, the most recent one.
THCRAP_API const char *runconfig_latest_get();

// Return the number of stages
THCRAP_API size_t runconfig_stage_count();
...
THCRAP_INTERNAL_API DWORD runconfig_loader_pid_get();

// Set the PID of the thcrap_loader process. For use in thcrap_loader
THCRAP_INTERNAL_API void runconfig_loader_pid_set(DWORD loader_pid);
...
THCRAP_API bool runconfig_stage_apply(size_t stage_num, int flags, HMODULE module);
...
THCRAP_API void runconfig_load(json_t *file, int flags);

// Load a run configuration from a file.
THCRAP_API void runconfig_load_from_file(const char *path);

// Free the run configuration.
// You can load a new run configuration after calling this function.
THCRAP_API void runconfig_free();


// Set the thcrap directory
THCRAP_API void runconfig_thcrap_dir_set(const char *thcrap_dir);

// Set the game build
THCRAP_API void runconfig_build_set(const char *build);
